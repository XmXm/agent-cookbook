---
name: pc-wsl-docker
description: Manage Docker containers and Compose stacks on the user's remote Windows PC that runs Docker inside WSL Debian, exposed to the LAN through Windows portproxy via `sync-ports.bat`. Use this whenever the user asks to start/stop/inspect a container, change a docker-compose stack, expose a service to the LAN, or do anything container-related on "PC", "我的电脑", "那台电脑", "the WSL box", "the home server", or any remote docker host reached via `ssh pc`. Trigger even when the user does not explicitly mention WSL or portproxy — if the task is container CRUD or compose management and the target is not the local machine, default to this skill.
---
...
Everything goes through one entry point: **`ssh pc`**.
...
| --- | --- |
| Shell on PC (lands in MINGW64 / git-bash, already Admin) | `ssh pc` |
| One-shot Windows-side command | `ssh pc '<bash-cmd>'` |
| One-shot Docker command in WSL | `ssh pc 'wsl -d Debian -- docker <args>'` |
| Multi-line bash inside WSL (login shell, paths resolved) | `ssh pc 'wsl -d Debian -- bash -lic "<cmd>"'` |
| Re-sync portproxy after any port change | `ssh pc 'sync-ports.bat'` |
| Preview what sync would do | `ssh pc 'sync-ports.bat -WhatIf'` |
...
- The script auto-elevates via UAC, but the `ssh pc` session already runs as Administrator, so it just works headless.
...
```bash
ssh pc 'wsl -d Debian -- docker ps -a'
ssh pc 'wsl -d Debian -- bash -lic "ls ~/docker/"'
```
...
```bash
ssh pc 'wsl -d Debian -- docker run -d --name foo -p 127.0.0.1:18080:80 nginx:alpine'
ssh pc 'wsl -d Debian -- docker logs -n 50 foo'
```

**For compose stacks**, work inside `~/docker/<stack>/`:

```bash
ssh pc 'wsl -d Debian -- bash -lic "mkdir -p ~/docker/myapp && cat > ~/docker/myapp/compose.yaml << EOF
services:
  app:
    image: ...
...
```bash
# Container is up
ssh pc 'wsl -d Debian -- docker ps --filter name=<container>'

# WSL loopback responds
ssh pc 'wsl -d Debian -- curl -s -o /dev/null -w "wsl=%{http_code}\n" http://127.0.0.1:<port>/'

# Windows public-facing portproxy is in place
ssh pc 'netsh interface portproxy show all'

# Windows side actually serves it
ssh pc 'curl -s -o /dev/null -w "win=%{http_code}\n" http://127.0.0.1:<port>/'
```
...
```bash
ssh pc 'wsl -d Debian -- bash -lic "cd ~/docker/<stack> && docker compose down"'
...
```bash
ssh pc 'wsl -d Debian -- docker rm -f <name>'
ssh pc 'sync-ports.bat'
```
...
| `sync-ports.bat` says "Administrator privileges required" | SSH session not elevated for some reason | Re-run from a fresh `ssh pc` — the session normally runs as Admin. |
...
| `wsl docker` hangs / errors right after Windows boot | WSL not yet started | `ssh pc 'wsl -d Debian -- echo ready'` to warm it, then retry. |
