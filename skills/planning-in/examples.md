...
**User:** `/planning-in ./plans/exercise-research`
...
```bash
/planning-in ./plans/auth-refactor
/planning-in ./plans/dark-mode
```
...
**User:** `/planning-in ./plans/fix-login-bug`
...
```bash
/planning-in ./plans/feature-x          # Create plan
# ... work on feature-x ...
/planning-in:status                      # Check all plans
# feature-x shows 5/5 phases complete
/planning-archive ./plans/feature-x      # Archive completed plan
```

After archive, `feature-x` is removed from `.planning-dir` and the directory is preserved for reference.
